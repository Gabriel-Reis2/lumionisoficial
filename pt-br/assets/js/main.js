// LUMIONIS - Script Principal (PT-BR)
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 5. Observer de Scroll para Animações
  ...
  // 6. Formulário de Contato Inteligente (PT-BR)
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      if (whatsapp) textoMsg += `*WhatsApp:* ${whatsapp}\n`;
      textoMsg += `*Serviço de Interesse:* ${servico}\n\n`;
      textoMsg += `*Mensagem:*\n${mensagem}`;

      const zapUrl = `https://wa.me/5521998563527?text=${encodeURIComponent(textoMsg)}`;
        ...
        formFeedback.innerHTML = `
          <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm flex flex-col gap-3">
            <div class="flex items-center gap-2 font-medium">
              ...
              <span>Mensagem estruturada com sucesso!</span>
            </div>
            <p class="text-xs text-zinc-300">
              Para resposta imediata com Gabriel (CEO da LUMIONIS), clique abaixo para abrir diretamente a conversa no WhatsApp:
            </p>
            <a href="${zapUrl}" target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs transition-colors">
              <span>Continuar atendimento no WhatsApp (+55 21 99856-3527)</span> →
            </a>
          </div>
        `;
